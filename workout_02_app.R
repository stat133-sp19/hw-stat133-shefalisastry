  ...
  output$distPlot <- renderPlot({
    ...
    if (input$facet == "No") {
      ggplot(melt_modalities, aes(x = year, y = value)) + 
        geom_line(aes(color = variable)) + ggtitle("Timelines")
    }
    else {
      ggplot(melt_modalities, aes(x = year, y = value)) + 
        geom_line(aes(color = variable)) + geom_area(aes(fill = variable, alpha = 0.5)) + facet_grid(~variable) + ggtitle("Timelines")
    }
    })
